          ...
          {experiences.map((exp, index) => (
            ...
            >
              <div className="flex flex-col">
                <span className="text-sm font-mono text-gray-medium mb-2 uppercase tracking-wider">{exp.period}</span>
                <span className={`text-xl font-bold text-foreground w-fit ${exp.company === "Hy Bro :)" ? "blur-sm select-none" : ""}`}>
                  {exp.company}
                </span>
              </div>
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-3 pt-2">
                  {exp.achievements.map((achievement, i) => (
                    <li key={i} className="flex gap-3 text-sm text-gray-medium leading-relaxed">
