...
export const TechIcons: Record<string, React.FC<{ className?: string }>> = {
  ...
  "Socket.io": ({ className }) => (
     <svg viewBox="0 0 467 467" className={className} xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" strokeLinejoin="round" strokeMiterlimit="1.414"><circle cx="233.197" cy="233.197" r="233.197" fill="none"/><path d="M233.197 0C104.53 0 0 104.53 0 233.197s104.53 233.197 233.197 233.197 233.197-104.53 233.197-233.197S361.864 0 233.197 0zm0 411.33c-98.17 0-178.13-79.96-178.13-178.13s79.96-178.13 178.13-178.13 178.13 79.96 178.13 178.13-79.96 178.13-178.13 178.13z" fill="#010101"/><path d="M309.845 140.732h-31.54c-3.13 0-5.753 2.622-5.753 5.752v176.657a5.55 5.55 0 01-5.553 5.554h-28.52a5.55 5.55 0 01-5.554-5.554V146.484a5.75 5.75 0 00-5.753-5.752h-29.07c-3.12 0-5.752 2.62-5.752 5.752v176.657a5.55 5.55 0 01-5.553 5.554h-29.35a5.55 5.55 0 01-5.554-5.554V126.31a5.61 5.61 0 00-5.613-5.613h-44.59c-3.23 0-5.762 2.62-5.762 5.852v213.627c0 3.232 2.53 5.852 5.763 5.852h241.69c3.232 0 5.852-2.62 5.852-5.852V146.584c0-3.23-2.62-5.852-5.852-5.852" fill="#010101"/></svg>
  ),
  "MySQL": ({ className }) => (
    <svg viewBox="0 0 24 24" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
       <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12" stroke="#00758F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
       <path d="M17 12H7" stroke="#F29111" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
};
  ...
  return (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      {technologies.map((tech) => {
        ...
        if (!Icon) return (
             <span key={tech} className="px-2 py-1 bg-gray-100 rounded text-xs text-gray-500 font-mono">
                {tech}
             </span>
        );
        ...
        return (
          <div key={tech} className="group flex items-center">
            <motion.div 
               className="flex items-center gap-2 bg-gray-50 border border-gray-100 p-1.5 rounded-lg cursor-default transition-colors hover:bg-white hover:border-gray-300"
               initial="collapsed"
               whileHover="expanded"
            >
